      ...
      <section className="border-b border-white/10 bg-[#1F2937]">
        <div className="container mx-auto px-4 py-10">
          <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
            <div>
              <p className="text-sm font-semibold uppercase tracking-widest text-[#10B981]">Programme complet</p>
              <h1 className="mt-2 text-3xl font-black text-[#F9FAFB] md:text-4xl">Toutes les sessions</h1>
              <p className="mt-3 text-gray-400">
                {sessions.length} session{sessions.length > 1 ? 's' : ''} disponible{sessions.length > 1 ? 's' : ''}, class̩e{sessions.length > 1 ? 's' : ''} par heure de d̩but.
              </p>
            </div>
            <Link
              href="/favourites"
      ...
      <section className="container mx-auto px-4 py-10">
        {sessions.length === 0 ? (
          <div className="rounded-lg border border-white/10 bg-[#1F2937] px-6 py-14 text-center shadow-sm">
            ...
            <h3 className="text-xl font-semibold text-[#F9FAFB]">Aucune session disponible</h3>
            <p className="mt-2 text-gray-400">Revenez plus tard pour d̩couvrir les nouvelles sessions.</p>
          </div>
        ) : (
